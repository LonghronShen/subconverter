...
// std::vector<responseRouteWithCallback> responses;
...
//       "Access-Control-Allow-Origin: *\r\nAccess-Control-Allow-Headers: *\r\n"
//       "Content-Length: " +
//       std::__cxx11::to_string(prompt_info.size()) + "\r\n\r\n" + prompt_info +
//       "\r\n";
...
//       "Content-Length: " + std::__cxx11::to_string(content.size()) + "\r\n";
//   sendall(sock, extra_header);
//   send(sock, "\r\n", 2, 0);
...
//   ret = setsockopt(s, SOL_SOCKET, SO_SNDTIMEO, (const char *)&timeo,
//                    sizeof(timeo));
//   ret = setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (const char *)&timeo,
//                    sizeof(timeo));
// #endif
//   def_timeout = timeout;
//   return ret;
...
//       Response response;
//       serve_content(client_sock, iter->content_type, rc(request, response));
//       goto end;
//     }
//   }
...
// void append_response(const std::string &type, const std::string &request,
//                      const std::string &content_type,
//                      response_callback response) {
//   responseRouteWithCallback rr;
//   rr.method = type;
//   rr.path = request;
...
// void stop_web_server() { SERVER_EXIT_FLAG = true; }
...
//   int port = args->port, max_conn = args->max_conn,
//       max_workers = args->max_workers, numbytes, worker_index = 0;
...
//   while (true) {
//     acc_socket = accept(server_socket, (struct sockaddr *)&user_socket,
//                         &sock_size); // wait for connection
//     if (acc_socket < 0) {
//       if (errno == EINTR || errno == EWOULDBLOCK || errno == EAGAIN) {
//         fail_counter++;
